...
clust_assign_hth <- chain_clust %>%
  filter(chain == best_chain) %>% 
  pull(assign)

shuffle_hth <- clue::solve_LSAP(table(clust_assign_true, clust_assign_hth), maximum = TRUE)
acctbl_hth <- (table(clust_assign_true, clust_assign_hth)[,shuffle_hth])
acc_hth <- (acctbl_hth %>% diag() %>% sum()) / N

tictoc::tic()
mod_lassomle <- runValse(stack_X, stack_Y, procedure = "LassoRank", verbose = TRUE, kmin = 2, kmax = 2)
time_valse <- tictoc::toc()

clust_assign_lassomle <- mod_lassomle$proba %>% 
  tibble() %>% 
  rowwise() %>% 
  mutate(across(where(is.numeric), which.max)) %>% 
  pull()

shuffle_lassomle <- clue::solve_LSAP(table(clust_assign_true, clust_assign_lassomle), maximum = TRUE)
acctbl_lassomle <- (table(clust_assign_true, clust_assign_lassomle)[,shuffle_lassomle])

acc_lassomle <- (acctbl_lassomle %>% diag() %>% sum()) / N



tibble(method = c("HTH", "LassoMLE"), 
       accuracy = c(acc_hth, acc_lassomle), 
       time = c((time_hth$toc-time_hth$tic), (time_valse$toc-time_valse$tic))) %>% 
  mutate(time = time/60)
