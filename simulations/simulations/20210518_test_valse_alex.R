...
clust_assign_lassomle <- test$proba %>% 
  tibble() %>% 
  rowwise() %>% 
  mutate(across(where(is.numeric), which.max)) %>% 
  pull()

shuffle <- clue::solve_LSAP(table(clust_assign_true, clust_assign_lassomle), maximum = TRUE)
acctbl <- (table(clust_assign_true, clust_assign_lassomle)[,shuffle])
